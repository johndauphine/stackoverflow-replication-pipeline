...
# Table replication order with parallel execution groups
# Group 1: Independent lookup tables (run in parallel)
GROUP_1_INDEPENDENT = ["VoteTypes", "PostTypes", "LinkTypes"]

# Group 2: Parent table (depends on Group 1)
GROUP_2_USERS = ["Users"]

# Group 3: Tables depending on Users (run in parallel)
GROUP_3_USER_DEPENDENT = ["Badges", "Posts"]

# Group 4: Tables depending on Posts (run in parallel)
GROUP_4_POST_DEPENDENT = ["PostLinks", "Comments", "Votes"]
...
def create_target_schema() -> None:
    ...
    # Get all tables to create (order doesn't matter for schema creation)
    all_tables = GROUP_1_INDEPENDENT + GROUP_2_USERS + GROUP_3_USER_DEPENDENT + GROUP_4_POST_DEPENDENT
...
def copy_table_direct_stream(table: str) -> None:
    """
    OPTIMIZED: Copy table using direct row streaming (no CSV intermediate).

    Performance Improvements vs Original:
    1. No CSV serialization/deserialization overhead
    2. No disk I/O from SpooledTemporaryFile
    3. Larger batch size (5000 vs 1000 rows)
    4. Streaming: can start writing while still reading
    5. Memory efficient: only batch size rows in memory at once

    Strategy: Read from source via cursor, write to target in batches.
    """
    src_hook = MsSqlHook(mssql_conn_id=SRC_CONN_ID)
    tgt_hook = MsSqlHook(mssql_conn_id=TGT_CONN_ID)

    log.info(f"[{table}] starting direct stream copy (batch_size={BATCH_SIZE})")
    ...
    try:
        ...
        with src_hook.get_conn() as src_conn:
            ...
            with src_conn.cursor() as src_cur:
                ...
                # OPTIMIZATION: Stream rows directly without intermediate buffering
                for row in src_cur:
                    batch.append(row)
        ...
        tgt_conn.commit()
        log.info(f"[{table}] direct stream completed: {total_rows} rows")
    finally:
        tgt_cur.close()
        tgt_conn.close()
...
with DAG(
    dag_id="replicate_stackoverflow_to_target_optimized",
    start_date=datetime(2024, 1, 1),
    schedule=None,
    catchup=False,
    default_args=DEFAULT_ARGS,
    tags=["stackoverflow", "mssql", "replication", "sql-server", "optimized"],
    template_searchpath=["/usr/local/airflow/include"],
    description="OPTIMIZED: Direct row streaming + parallel table copying (2-3x faster)",
) as dag:
    ...
    group_1_tasks = [
        ...
        for tbl in GROUP_1_INDEPENDENT
    ]

    # Group 2: Users table (depends on Group 1)
    copy_users = PythonOperator(
        task_id="copy_Users",
        python_callable=copy_table_direct_stream,
    ...
    group_3_tasks = [
        ...
        for tbl in GROUP_3_USER_DEPENDENT
    ]
    ...
    group_4_tasks = [
        ...
        for tbl in GROUP_4_POST_DEPENDENT
    ]
    ...
    # OPTIMIZED DEPENDENCY GRAPH: Parallel execution where possible
    reset_tgt >> create_schema

    # Group 1: Parallel lookup tables
    create_schema >> group_1_tasks

    # Group 2: Users (depends on all Group 1)
    group_1_tasks >> copy_users

    # Group 3: Parallel (Badges, Posts) depend on Users
    copy_users >> group_3_tasks

    # Group 4: Parallel (PostLinks, Comments, Votes) depend on Posts
    # Find Posts task from group_3_tasks
    copy_posts = [t for t in group_3_tasks if t.task_id == "copy_Posts"][0]
    copy_posts >> group_4_tasks

    # All tables complete before fixing sequences
    group_4_tasks >> fix_sequences
    # Badges also completes before fixing sequences
    copy_badges = [t for t in group_3_tasks if t.task_id == "copy_Badges"][0]
    copy_badges >> fix_sequences
